...
with st.sidebar:
    ...
    glass_cost_per_m2 = st.number_input("Glass cost per m²", min_value=0.0, step=0.1, value=float(st.session_state.get("glass_cost_per_m2_value",0.0)), key="glass_cost_per_m2_value")
...
# Window schedule and BOM export datasets
...
window_schedule_df = pd.DataFrame(window_schedule_rows)

if 'profile_totals' in locals() and isinstance(profile_totals, pd.DataFrame) and not profile_totals.empty:
    bom_profiles_df = profile_totals.copy()
else:
    bom_profiles_df = pd.DataFrame(columns=["profile", "total_cut_qty", "total_cut_length_mm", "total_cut_length_m", "weight_per_m_kg", "cost_per_kg", "estimated_weight_kg", "estimated_cost"])

# Build glass BOM directly from glass_rows so it always exists when glass pieces are calculated
glass_piece_df_for_bom = pd.DataFrame(glass_rows)
if not glass_piece_df_for_bom.empty:
    if "area_m2" in glass_piece_df_for_bom.columns and "qty" in glass_piece_df_for_bom.columns and "spec" in glass_piece_df_for_bom.columns:
        bom_glass_df = (
            glass_piece_df_for_bom.groupby("spec", dropna=False)
            .agg(
                total_pieces=("qty", "sum"),
                total_area_m2=("area_m2", lambda s: float((s * glass_piece_df_for_bom.loc[s.index, "qty"]).sum())),
            )
            .reset_index()
        )
    elif "spec" in glass_piece_df_for_bom.columns:
        bom_glass_df = (
            glass_piece_df_for_bom.groupby("spec", dropna=False)
            .size()
            .reset_index(name="total_pieces")
        )
        bom_glass_df["total_area_m2"] = 0.0
    else:
        bom_glass_df = pd.DataFrame(columns=["spec", "total_pieces", "total_area_m2", "estimated_cost"])
    if not bom_glass_df.empty:
        bom_glass_df["estimated_cost"] = bom_glass_df["total_area_m2"].apply(lambda x: round(float(x) * float(st.session_state.get("glass_cost_per_m2_value", 0.0) or 0), 2))
else:
    bom_glass_df = pd.DataFrame(columns=["spec", "total_pieces", "total_area_m2", "estimated_cost"])

hardware_bom_df = pd.DataFrame(columns=["item", "qty", "notes"])
...
glass_cost_total = round(float(summary.get("total_glass_area_m2",0) or 0) * float(st.session_state.get("glass_cost_per_m2_value",0) or 0),2)
...
with tab5:
    ...
    st.markdown('<div class="soft-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">Bill of Materials — Profiles</div>', unsafe_allow_html=True)
    if not bom_profiles_df.empty:
        show_cols = [c for c in ["profile", "total_cut_qty", "total_cut_length_mm", "total_cut_length_m", "weight_per_m_kg", "cost_per_kg", "estimated_weight_kg", "estimated_cost"] if c in bom_profiles_df.columns]
        st.dataframe(bom_profiles_df[show_cols], use_container_width=True, hide_index=True)
    else:
        st.info("No profile BOM available.")
    st.markdown('</div>', unsafe_allow_html=True)

    st.markdown('<div class="soft-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">Bill of Materials — Glass</div>', unsafe_allow_html=True)
    if not bom_glass_df.empty:
        show_cols = [c for c in ["spec", "total_pieces", "total_area_m2", "estimated_cost"] if c in bom_glass_df.columns]
        st.dataframe(bom_glass_df[show_cols], use_container_width=True, hide_index=True)
    else:
        st.info("No glass BOM available.")
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    total_glass_cost = float(bom_glass_df["estimated_cost"].sum()) if ("estimated_cost" in bom_glass_df.columns and not bom_glass_df.empty) else 0.0

    st.markdown('<div class="soft-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">Schedule & BOM Exports</div>', unsafe_allow_html=True)
    ...
    with d2:
        st.download_button(
            "Download Window Schedule CSV",
            data=window_schedule_df.to_csv(index=False).encode("utf-8"),
            file_name=f"{project_name.strip().replace(' ', '_') or 'project'}_window_schedule.csv",
            mime="text/csv",
            use_container_width=True,
        )
    with d3:
        combined_bom = {
            "window_schedule_rows": int(len(window_schedule_df)),
            "profile_bom_rows": int(len(bom_profiles_df)),
            "glass_bom_rows": int(len(bom_glass_df)),
            "profiles_cost": total_profiles_cost,
            "glass_cost": total_glass_cost,
            "combined_cost": total_profiles_cost + total_glass_cost,
        }
        st.download_button(
            "Download BOM Summary JSON",
            data=json.dumps(combined_bom, indent=2),
            file_name=f"{project_name.strip().replace(' ', '_') or 'project'}_bom_summary.json",
