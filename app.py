...
def compute_project_material_dashboard(summary: dict, stock_length_mm: float, glass_sheet_width_mm: float, glass_sheet_height_mm: float):
    aluminium_rate = float(st.session_state.get("aluminium_cost_per_kg_value", 0.0) or 0)
    glass_rate_sqft = float(st.session_state.get("glass_cost_per_sqft_value", 0.0) or 0)
    profiles_weight_kg = float(summary.get("estimated_weight_kg", 0) or 0)
...
with tab3:
    ...
    dashboard = compute_project_material_dashboard(summary, stock_length_mm, glass_sheet_width_mm, glass_sheet_height_mm)

    st.markdown('<div class="soft-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">Project material dashboard</div>', unsafe_allow_html=True)
    d1, d2, d3 = st.columns(3)
    with d1:
        render_metric("Profiles weight", f"{dashboard['profiles_weight_kg']} kg")
        st.markdown(f"<div style='margin-top:8px;color:#475569;font-size:0.92rem;'><b>Profiles cost:</b> {money_fmt(dashboard['profiles_cost'])}</div>", unsafe_allow_html=True)
    with d2:
        render_metric("Glass sheets", str(dashboard["sheets_to_purchase"]))
        st.markdown(f"<div style='margin-top:8px;color:#475569;font-size:0.92rem;'><b>Glass purchase area:</b> {dashboard['glass_purchase_area_m2']} m²</div>", unsafe_allow_html=True)
        st.markdown(f"<div style='margin-top:4px;color:#475569;font-size:0.92rem;'><b>Glass cost:</b> {money_fmt(dashboard['glass_cost'])}</div>", unsafe_allow_html=True)
    with d3:
        render_metric("Total material", money_fmt(dashboard["total_material_cost"]))
        st.markdown(f"<div style='margin-top:8px;color:#475569;font-size:0.92rem;'><b>Aluminium rate:</b> {money_fmt(st.session_state.get('aluminium_cost_per_kg_value', 0.0))} / kg</div>", unsafe_allow_html=True)
        st.markdown(f"<div style='margin-top:4px;color:#475569;font-size:0.92rem;'><b>Glass rate:</b> {money_fmt(st.session_state.get('glass_cost_per_sqft_value', 0.0))} / sq ft</div>", unsafe_allow_html=True)
