...
# Safer aluminium weight calculation using prefix matching from profile rules to workbook system weights
...
aluminium_cost_total = round(float(calculated_aluminium_weight_kg or 0) * float(st.session_state.get("aluminium_cost_per_kg_value",0) or 0), 2)
...
if not profile_piece_df.empty:
    ...
    profile_totals["estimated_cost"] = profile_totals["estimated_weight_kg"].apply(lambda x: round(float(x) * float(st.session_state.get("aluminium_cost_per_kg_value", 0) or 0), 2))
else:
...
with tab3:
    ...
    if missing_weight_profiles:
        st.warning("Some profiles do not have weight data in the workbook weight map. Their weight and cost are shown as 0 until weights are added: " + ", ".join(missing_weight_profiles))
    ...
    if not profile_totals.empty:
        show_cols = [c for c in ["profile", "total_cut_qty", "total_cut_length_mm", "total_cut_length_m", "weight_per_m_kg", "estimated_weight_kg", "estimated_cost"] if c in profile_totals.columns]
        st.dataframe(
