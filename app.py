...
aluminium_cost_total = round(float(summary.get("estimated_weight_kg",0) or 0) * float(st.session_state.get("aluminium_cost_per_kg_value",0)),2)
glass_cost_total = round(float(summary.get("total_glass_area_m2",0) or 0) * float(st.session_state.get("glass_cost_per_m2_value",0)),2)
...
with tab3:
    ...
    with c1: render_metric("Windows", str(summary["window_lines"]))
    with c2: render_metric("Profile cuts", str(summary["profile_cuts"]))
    with c3: render_metric("New aluminium bars", str(summary["aluminium_new_bars"]))
    with c4: render_metric("New glass sheets", str(summary["glass_new_sheets"]))
    ...
    st.metric("Estimated aluminium weight", f"{round(summary['estimated_weight_kg'],2)} kg")
    ...
    with p1: st.metric("Aluminium purchase cost", f"{aluminium_cost_total:,.2f}")
    with p2: st.metric("Glass purchase cost", f"{glass_cost_total:,.2f}")
    with p3: st.metric("Total material cost", f"{aluminium_cost_total + glass_cost_total:,.2f}")
