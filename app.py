...
with tab5:
    ...
    if 'order_breakdown' in locals() and isinstance(order_breakdown, pd.DataFrame) and not order_breakdown.empty:
        ...
        bom_profiles_df = pd.DataFrame(columns=["profile", "lengths_to_order", "ordered_length_mm", "ordered_length_m", "used_length_mm", "used_length_m", "waste_mm", "waste_m"])
    ...
    if not glass_sheet_piece_df.empty:
        ...
        if "placed_width_mm" in glass_sheet_piece_df.columns:
            glass_sheet_piece_df["cut_width_mm"] = glass_sheet_piece_df["placed_width_mm"]
        else:
            glass_sheet_piece_df["cut_width_mm"] = glass_sheet_piece_df.get("width_mm", 0)
        if "placed_height_mm" in glass_sheet_piece_df.columns:
            glass_sheet_piece_df["cut_height_mm"] = glass_sheet_piece_df["placed_height_mm"]
        else:
            glass_sheet_piece_df["cut_height_mm"] = glass_sheet_piece_df.get("height_mm", 0)
        ...
        if "glass_cost_per_sqft_value" in st.session_state:
            bom_glass_summary_df["estimated_cost"] = bom_glass_summary_df["total_area_m2"].apply(
                lambda x: round(float(x) * 10.7639 * float(st.session_state.get("glass_cost_per_sqft_value", 0.0) or 0), 2)
            )
        else:
            bom_glass_summary_df["estimated_cost"] = bom_glass_summary_df["total_area_m2"].apply(
        ...
        glass_sheet_piece_df = pd.DataFrame(columns=["piece_id", "window_label", "spec", "sheet_no", "cut_width_mm", "cut_height_mm", "area_m2_calc"])
    ...
    if not bom_profiles_df.empty:
        show_cols = [c for c in ["profile", "lengths_to_order", "bars_to_order", "ordered_length_mm", "ordered_length_m", "used_length_mm", "used_length_m", "waste_mm", "waste_m"] if c in bom_profiles_df.columns]
