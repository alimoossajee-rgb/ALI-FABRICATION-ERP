...
# Safer aluminium weight calculation so missing profile weights do not blank the totals
...
if not profile_piece_df_for_weight.empty and "profile" in profile_piece_df_for_weight.columns:
    def _row_weight_kg(row):
        ...
        weight_per_m = catalog.weights.get(profile)
        if weight_per_m in [None, ""] or pd.isna(weight_per_m):
...
with tab3:
    ...
    if not profile_totals.empty:
        st.dataframe(
            profile_totals[["profile", "total_cut_qty", "total_cut_length_mm", "total_cut_length_m", "weight_per_m_kg", "estimated_weight_kg", "estimated_cost"]],
            use_container_width=True,
            hide_index=True,
        )
    else:
