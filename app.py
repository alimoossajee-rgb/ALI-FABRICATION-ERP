...
def lookup_profile_weight(profile_name: str, weights_map: dict) -> float:
    ...
    for k, v in weights_map.items():
        ...
        if profile_norm.startswith(kn) and len(kn) > best_len:
            best_len = len(kn)
            best_val = float(v)
    return best_val
...
:root {{
    ...
    --shadow-deep: 0 20px 55px rgba(15,23,42,0.16);
}}
.stApp {{
    background:
        radial-gradient(circle at 15% 10%, rgba(96,165,250,0.20), transparent 22%),
        radial-gradient(circle at 85% 10%, rgba(251,146,60,0.18), transparent 20%),
...
[data-testid="stSidebar"] {{
    background: linear-gradient(180deg, rgba(255,255,255,0.74), rgba(248,250,252,0.66));
...
.brand-hero {{
    ...
    padding: 30px 34px;
    margin-bottom: 1rem;
    background: linear-gradient(135deg, rgba(30,64,175,0.72) 0%, rgba(15,23,42,0.68) 58%, rgba(251,146,60,0.66) 100%);
    color: white;
...
.preview-img {{
    ...
    max-height:240px;
    object-fit:contain;
    background: linear-gradient(180deg, rgba(255,255,255,0.70), rgba(248,250,252,0.52));
    border-radius:18px;
    padding:10px;
    border:1px solid rgba(255,255,255,0.42);
}}
...
.stButton>button {{
    background: linear-gradient(135deg, rgba(59,130,246,0.82), rgba(37,99,235,0.78)) !important;
    color:white !important;
}}
.stDownloadButton>button {{
    background: linear-gradient(135deg, rgba(251,146,60,0.86), rgba(249,115,22,0.82)) !important;
    color:white !important;
}}
...
@media (max-width: 900px) {{
    ...
    .brand-hero {{ padding:18px 16px; border-radius:24px; }}
    .hero-wrap {{ gap:12px; align-items:flex-start; }}
    .hero-logo {{ width:66px; height:66px; padding:6px; }}
    .hero-title {{ font-size:1.38rem; }}
    .hero-sub {{ font-size:.90rem; }}
    .soft-card {{ padding:14px 12px 8px 12px; border-radius:18px; }}
    .preview-img {{ max-height:165px; }}
    .metric-value {{ font-size:1.28rem; }}
    .board-card {{ min-height:292px; padding:12px; border-radius:20px; }}
    .board-meta {{ grid-template-columns:1fr; }}
    .soft-card, .metric-card, .board-card, .dashboard-strip, .preview-shell {{ color: #0f172a !important; }}
    .soft-card *, .metric-card *, .board-card *, .dashboard-strip *, .preview-shell * {{ color: inherit !important; }}
    ...
    .stDataFrame, [data-testid="stDataFrame"], [data-testid="stDataFrame"] * {{ color: #0f172a !important; }}
}}
</style>
...
tab0, tab1, tab2, tab3, tab4, tab5 = st.tabs(["🏗 Project Board", "🪟 Window Studio", "♻ Offcuts", "📊 Results", "✂ Visual Layouts", "📦 Schedule & BOM"])
...
if not profile_piece_df_for_weight.empty and "profile" in profile_piece_df_for_weight.columns:
    ...
    summary["estimated_weight_kg"] = round(float(profile_piece_df_for_weight.apply(_row_weight_kg, axis=1).sum()), 2)
    aluminium_cost_total = round(float(profile_piece_df_for_weight.apply(_row_cost, axis=1).sum()), 2)
else:
    summary["estimated_weight_kg"] = 0.0
...
with tab5:
    ...
    if isinstance(order_breakdown, pd.DataFrame) and not order_breakdown.empty:
    ...
    total_glass_cost = float(bom_glass_summary_df["estimated_cost"].fillna(0).sum()) if ("estimated_cost" in bom_glass_summary_df.columns and not bom_glass_summary_df.empty) else 0.0
    ...
    workbook_payload = build_schedule_bom_workbook(window_schedule_df, bom_profiles_df, bom_glass_summary_df)
    st.download_button(
