...
def build_schedule_bom_workbook(window_schedule_df: pd.DataFrame, bom_profiles_df: pd.DataFrame, bom_glass_summary_df: pd.DataFrame) -> bytes:
    output = BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        bom_glass_summary_df.to_excel(writer, sheet_name="BOM Glass", index=False)
...
with tab5:
    ...
    if not glass_sheet_piece_df.empty:
        ...
        if "qty" not in glass_sheet_piece_df.columns:
        ...
        bom_glass_summary_df["estimated_cost"] = bom_glass_summary_df["total_area_m2"].apply(lambda x: round(float(x) * 10.7639 * float(st.session_state.get("glass_cost_per_sqft_value", 0.0) or 0), 2))
    else:
    ...
    if not glass_sheet_piece_df.empty:
        detail_cols = [c for c in ["piece_id", "window_label", "spec", "sheet_no", "cut_width_mm", "cut_height_mm", "area_m2_calc"] if c in glass_sheet_piece_df.columns]
        st.dataframe(glass_sheet_piece_df[detail_cols], use_container_width=True, hide_index=True)
    ...
    total_glass_cost = float(bom_glass_summary_df["estimated_cost"].sum()) if "estimated_cost" in bom_glass_summary_df.columns and not bom_glass_summary_df.empty else 0.0
