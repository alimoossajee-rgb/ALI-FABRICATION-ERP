...
with tab5:
    ...
    if not glass_piece_df_bom.empty:
        if "area_m2" in glass_piece_df_bom.columns and "qty" in glass_piece_df_bom.columns and "spec" in glass_piece_df_bom.columns:
            bom_glass_df = (
                glass_piece_df_bom.groupby("spec", dropna=False)
                .agg(
                    total_pieces=("qty", "sum"),
                    total_area_m2=("area_m2", lambda s: float((s * glass_piece_df_bom.loc[s.index, "qty"]).sum())),
                )
                .reset_index()
            )
        elif "spec" in glass_piece_df_bom.columns:
            bom_glass_df = (
                glass_piece_df_bom.groupby("spec", dropna=False)
                .size()
