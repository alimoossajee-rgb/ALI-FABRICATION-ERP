...
with tab5:
    ...
    # Build schedule and BOM data here so this tab never depends on variables defined earlier in the file.
    schedule_rows = []
    ...
    glass_piece_df_bom = pd.DataFrame(glass_rows)
    if not glass_piece_df_bom.empty and "spec" in glass_piece_df_bom.columns:
        if "qty" not in glass_piece_df_bom.columns:
            glass_piece_df_bom["qty"] = 1
        # Build area directly from width/height when area_m2 is missing or unreliable
        if "area_m2" not in glass_piece_df_bom.columns or glass_piece_df_bom["area_m2"].fillna(0).sum() == 0:
            if "width_mm" in glass_piece_df_bom.columns and "height_mm" in glass_piece_df_bom.columns:
                glass_piece_df_bom["area_m2_calc"] = (
                    glass_piece_df_bom["width_mm"].fillna(0).astype(float)
                    * glass_piece_df_bom["height_mm"].fillna(0).astype(float)
                    / 1000000.0
                )
            else:
                glass_piece_df_bom["area_m2_calc"] = 0.0
        else:
            glass_piece_df_bom["area_m2_calc"] = glass_piece_df_bom["area_m2"].fillna(0).astype(float)

        bom_glass_df = (
            glass_piece_df_bom.groupby("spec", dropna=False)
            .agg(
                total_pieces=("qty", "sum"),
                total_area_m2=("area_m2_calc", lambda s: float((s * glass_piece_df_bom.loc[s.index, "qty"]).sum())),
            )
            .reset_index()
        )
        bom_glass_df["estimated_cost"] = bom_glass_df["total_area_m2"].apply(
            lambda x: round(float(x) * float(st.session_state.get("glass_cost_per_m2_value", 0.0) or 0), 2)
        )
    else:
