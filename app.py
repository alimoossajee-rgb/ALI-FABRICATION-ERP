...
tab0, tab1, tab2, tab3, tab4 = st.tabs(["Project Board", "Window Entry", "Offcuts", "Results", "Visual Layouts"])
...
# Add weight/cost summary columns when workbook weights are available
if not profile_totals.empty and hasattr(catalog, "weights"):
    def _lookup_weight(profile_name):
        try:
            return float((catalog.weights or {}).get(profile_name, 0) or 0)
        except Exception:
            return 0.0
    profile_totals["weight_per_m_kg"] = profile_totals["profile"].apply(_lookup_weight)
    profile_totals["cost_per_kg"] = float(st.session_state.get("aluminium_cost_per_kg_value", 0.0) or 0)
    ...
    if "weight_per_m_kg" not in profile_totals.columns:
        profile_totals["weight_per_m_kg"] = 0.0
    if "cost_per_kg" not in profile_totals.columns:
        profile_totals["cost_per_kg"] = float(st.session_state.get("aluminium_cost_per_kg_value", 0.0) or 0)
    if "estimated_weight_kg" not in profile_totals.columns:
        profile_totals["estimated_weight_kg"] = 0.0
    if "estimated_cost" not in profile_totals.columns:
        profile_totals["estimated_cost"] = 0.0

# Glass summary and purchase totals
...
aluminium_cost_total = float(profile_totals["estimated_cost"].sum()) if "estimated_cost" in profile_totals.columns and not profile_totals.empty else float(summary.get("estimated_weight_kg", 0) or 0) * float(st.session_state.get("aluminium_cost_per_kg_value", 0.0) or 0)
...
with tab3:
    ...
    if 'missing_weight_profiles' in locals() and missing_weight_profiles:
    ...
    if 'profile_totals' in locals() and isinstance(profile_totals, pd.DataFrame) and not profile_totals.empty:
    ...
    if 'glass_summary' in locals() and isinstance(glass_summary, pd.DataFrame) and not glass_summary.empty:
    ...
    elif "glass_cost_per_m2_value" in st.session_state:
        st.write(f"**Glass cost per m²:** {float(st.session_state.get('glass_cost_per_m2_value', 0.0) or 0):,.2f}")
    st.markdown('</div>', unsafe_allow_html=True)
