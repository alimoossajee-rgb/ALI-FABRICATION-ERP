...
def lookup_profile_weight(profile_name: str, weights_map: dict) -> float:
    ...
    for k, v in weights_map.items():
        ...
        if profile_norm.startswith(kn) and len(kn) > len(best_key):
            best_key = kn
            best_val = float(v)
    return float(best_val or 0.0)


def lookup_profile_cost_per_kg(profile_name: str) -> float:
    return float(st.session_state.get("aluminium_cost_per_kg_value", 0.0) or 0.0)
    ...
    css = """
        ...
        .stApp {{
            background:
                ...
                radial-gradient(circle at left top, rgba(251,146,60,0.10), transparent 18%),
                linear-gradient(180deg, #eef5ff 0%, #f8fbff 50%, #eef4ff 100%);
            color: var(--ink);
        }}
        ...
        .brand-hero {{
            background: linear-gradient(135deg, rgba(30,64,175,0.78) 0%, rgba(30,41,59,0.72) 58%, rgba(251,146,60,0.66) 100%);
            color: white;
            border-radius: 30px;
            padding: 28px 32px;
        ...
        .metric-card, .soft-card, .board-card, .preview-shell, .board-kpi, .dashboard-strip, div[data-testid="stExpander"] {{
        ...
        .drawing-shell {{
            background: linear-gradient(180deg, rgba(255,255,255,0.62), rgba(248,250,252,0.46));
            border:1px solid rgba(255,255,255,0.40);
            border-radius:18px;
            padding:8px;
        ...
        .stButton>button {{
            ...
            background: linear-gradient(135deg, rgba(96,165,250,0.88), rgba(59,130,246,0.78)) !important;
        ...
        .stDownloadButton>button {{
            border-radius: 14px !important;
            border: 1px solid rgba(255,255,255,0.28) !important;
            background: linear-gradient(135deg, rgba(251,146,60,0.84), rgba(249,115,22,0.78)) !important;
            color: white !important;
            font-weight: 800 !important;
            box-shadow: 0 8px 20px rgba(15,23,42,0.10);
        }}
        ...
        @media (max-width: 900px) {{
            ...
            .soft-card, .metric-card, .board-card, .dashboard-strip, .preview-shell {{
                color: #0f172a !important;
            }}
            .soft-card *, .metric-card *, .board-card *, .dashboard-strip *, .preview-shell * {{
                color: inherit !important;
            }}
            .stDataFrame, [data-testid="stDataFrame"], [data-testid="stDataFrame"] * {{
                color: #0f172a !important;
            }}
        }}
        </style>
...
def render_aluminium_bar_layouts(aluminium: dict):
    ...
    for bar in bars:
        ...
        st.markdown(
            ...
            <div class="soft-card">
                <div style="display:flex;justify-content:space-between;gap:12px;align-items:center;flex-wrap:wrap;margin-bottom:10px;">
                    <div style="font-weight:800;color:#1e40af;">Bar {bar.get("bar_no","")}</div>
        ...
        for idx, cut in enumerate(cuts, start=1):
            ...
            segs.append(
                f'<div style="width:{width_pct:.4f}%;min-width:86px;height:96px;background:#dbeafe;border-right:1px solid white;display:flex;align-items:center;justify-content:center;text-align:center;font-size:10px;font-weight:700;color:#1e3a8a;padding:4px;overflow:hidden;line-height:1.08;">{win}<br>{profile_name[:24]}<br>{int(length)} mm<br>{angle}</div>'
            )
            rows.append({"Cut No": idx, "Window": win, "Profile": profile_name, "Length (mm)": int(length), "Angle": angle, "Source length (mm)": int(stock)})
            running += length
            if idx < len(cuts):
                ...
                markers.append(
                    ...
                    f'<div style="position:absolute;left:calc({left_pct:.4f}% - 22px);top:0;background:#1e40af;color:white;border-radius:999px;padding:3px 8px;font-size:11px;font-weight:800;">{angle}</div>'
                )
        if waste > 0:
            waste_pct = max((waste / stock) * 100.0, 2.0)
            segs.append(
                f'<div style="width:{waste_pct:.4f}%;min-width:58px;height:96px;background:#fee2e2;display:flex;align-items:center;justify-content:center;text-align:center;font-size:11px;font-weight:700;color:#991b1b;padding:4px;line-height:1.1;">Waste<br>{int(waste)} mm</div>'
            )
        ...
        <div class="soft-card" style="padding:14px 16px;">
            <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;">
                <div style="color:#475569;font-size:0.95rem;">
                    Navigate the whole project visually, jump into any window instantly, and manage repeated systems without scrolling through long forms.
                </div>
                <div class="dashboard-chip">Visual Navigator</div>
            </div>
        </div>
        ''',
        unsafe_allow_html=True,
...
with tab1:
    st.markdown('<div class="section-title">Window Studio</div>', unsafe_allow_html=True)
    st.markdown(
        '''
        <div class="soft-card" style="padding:14px 16px;">
            <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;">
                <div style="color:#475569;font-size:0.95rem;">
                    Build each window using system-specific inputs only. The visual reference remains on the right while the fabrication logic updates automatically.
                </div>
                <div class="dashboard-chip">System-Specific Forms</div>
            </div>
        </div>
        ''',
        unsafe_allow_html=True,
...
if not profile_piece_df_for_weight.empty and "profile" in profile_piece_df_for_weight.columns:
    def _row_weight_kg(row):
        ...
        weight_per_m = lookup_profile_weight(profile, catalog.weights)
        if not weight_per_m:
    ...
    aluminium_cost_total = round(float(profile_piece_df_for_weight.apply(_row_al_cost, axis=1).sum()), 2)
else:
...
if not profile_piece_df.empty:
    ...
    profile_totals["weight_per_m_kg"] = profile_totals["profile"].apply(lambda p: lookup_profile_weight(p, catalog.weights))
    ...
    profile_totals = pd.DataFrame(columns=["profile", "total_cut_qty", "total_cut_length_mm", "total_cut_length_m", "weight_per_m_kg", "cost_per_kg", "estimated_weight_kg", "estimated_cost"])
...
if not glass_piece_df.empty and "spec" in glass_piece_df.columns:
    ...
    if "area_m2" not in glass_piece_df.columns or glass_piece_df["area_m2"].fillna(0).sum() == 0:
        if "width_mm" in glass_piece_df.columns and "height_mm" in glass_piece_df.columns:
            glass_piece_df["area_m2_calc"] = glass_piece_df["width_mm"].astype(float).fillna(0) * glass_piece_df["height_mm"].astype(float).fillna(0) / 1000000.0
        else:
            glass_piece_df["area_m2_calc"] = 0.0
    else:
        glass_piece_df["area_m2_calc"] = glass_piece_df["area_m2"].astype(float).fillna(0)
    glass_summary = (
        glass_piece_df.groupby("spec", dropna=False)
        .agg(total_pieces=("qty", "sum"), total_area_m2=("area_m2_calc", lambda s: float((s * glass_piece_df.loc[s.index, "qty"]).sum())))
        .reset_index()
    )
    glass_summary["estimated_cost"] = glass_summary["total_area_m2"].apply(lambda x: round(float(x) * SQFT_PER_M2 * float(st.session_state.get("glass_cost_per_sqft_value", 0.0) or 0), 2))
else:
    glass_summary = pd.DataFrame(columns=["spec", "total_pieces", "total_area_m2", "estimated_cost"])
...
with tab3:
    st.markdown(
        ...
        <div class="soft-card" style="padding:14px 16px;">
            <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;">
                <div style="color:#475569;font-size:0.95rem;">
                    Review ordered profile requirements, offcut usage, glass purchasing, and cost outputs in one structured view.
                </div>
                <div class="dashboard-chip">Procurement View</div>
            </div>
        </div>
        ''',
        unsafe_allow_html=True,
    ...
    with r1:
        ...
        if missing_weight_profiles:
            st.warning("Some profiles do not have a matching weight mapping yet: " + ", ".join(missing_weight_profiles))
...
with tab5:
    ...
    st.markdown(
        ...
        <div class="soft-card" style="padding:14px 16px;">
            <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;">
                <div style="color:#475569;font-size:0.95rem;">
                    Review the window schedule, purchasing quantities, and material BOM in one place, then export everything to Excel.
                </div>
                <div class="dashboard-chip">Procurement Pack</div>
            </div>
        </div>
        ''',
        unsafe_allow_html=True,
    ...
    bom_profiles_df = order_breakdown.copy() if isinstance(order_breakdown, pd.DataFrame) and not order_breakdown.empty else pd.DataFrame(
        columns=["profile","bars_to_order","ordered_length_mm","ordered_length_m","used_length_mm","used_length_m","waste_mm","waste_m"]
    )
    if not bom_profiles_df.empty:
        bom_profiles_df["lengths_to_order"] = bom_profiles_df["bars_to_order"]
        bom_profiles_df["weight_per_m_kg"] = bom_profiles_df["profile"].apply(lambda p: lookup_profile_weight(p, catalog.weights))
    ...
    if not glass_sheet_piece_df.empty:
        ...
        glass_sheet_piece_df["area_m2_calc"] = glass_sheet_piece_df["cut_width_mm"].astype(float).fillna(0) * glass_sheet_piece_df["cut_height_mm"].astype(float).fillna(0) / 1000000.0
        glass_sheet_piece_df["qty"] = 1
        bom_glass_detail_df = glass_sheet_piece_df.copy()
        bom_glass_df = (
            glass_sheet_piece_df.groupby("spec", dropna=False)
            .agg(total_pieces=("qty","sum"), total_area_m2=("area_m2_calc","sum"))
            .reset_index()
        )
        bom_glass_df["estimated_cost"] = bom_glass_df["total_area_m2"].apply(lambda x: round(float(x) * SQFT_PER_M2 * float(st.session_state.get("glass_cost_per_sqft_value", 0.0) or 0), 2))
    else:
        bom_glass_detail_df = pd.DataFrame(columns=["piece_id","window_label","spec","sheet_no","cut_width_mm","cut_height_mm","area_m2_calc"])
        bom_glass_df = pd.DataFrame(columns=["spec","total_pieces","total_area_m2","estimated_cost"])
    ...
    if not bom_profiles_df.empty:
        cols = [c for c in ["profile","lengths_to_order","ordered_length_mm","ordered_length_m","weight_per_m_kg","cost_per_kg","estimated_weight_kg","estimated_cost","used_length_mm","used_length_m","waste_mm","waste_m"] if c in bom_profiles_df.columns]
        st.dataframe(bom_profiles_df[cols], use_container_width=True, hide_index=True)
    else:
    ...
    if not bom_glass_detail_df.empty:
        cols = [c for c in ["piece_id","window_label","spec","sheet_no","cut_width_mm","cut_height_mm","area_m2_calc"] if c in bom_glass_detail_df.columns]
        st.dataframe(bom_glass_detail_df[cols], use_container_width=True, hide_index=True)
        st.markdown("**Glass purchase summary**")
        st.dataframe(bom_glass_df, use_container_width=True, hide_index=True)
    else:
        st.info("No glass needs to be cut from new sheets. All glass pieces were covered by offcuts.")
    st.markdown('</div>', unsafe_allow_html=True)

    total_profiles_cost = float(bom_profiles_df["estimated_cost"].fillna(0).sum()) if ("estimated_cost" in bom_profiles_df.columns and not bom_profiles_df.empty) else 0.0
    total_glass_cost = float(bom_glass_df["estimated_cost"].fillna(0).sum()) if ("estimated_cost" in bom_glass_df.columns and not bom_glass_df.empty) else 0.0
    ...
    workbook_bytes = build_schedule_bom_workbook(window_schedule_df, bom_profiles_df, bom_glass_df)
    st.download_button(
