...
def apply_project_payload(payload: dict, default_variant: str, default_glass_spec: str, variant_map: dict[str, str]):
    ...
    st.session_state.aluminium_cost_per_kg_value = float(payload.get("aluminium_cost_per_kg", 0.0))
    st.session_state.glass_cost_per_sqft_value = float(payload.get("glass_cost_per_sqft", 0.0))
    st.session_state.profile_cost_map = payload.get("profile_cost_map", [])
...
def project_payload_from_state(project_name: str, client_name: str, finish: str, stock_length_mm: float, glass_sheet_width_mm: float, glass_sheet_height_mm: float, kerf_mm: float, default_row_glass: str) -> dict:
    return {
        ...
        "aluminium_cost_per_kg": st.session_state.get("aluminium_cost_per_kg_value", 0.0),
        "glass_cost_per_sqft": st.session_state.get("glass_cost_per_sqft_value", 0.0),
        "profile_cost_map": st.session_state.get("profile_cost_map", []),
...
def lookup_profile_weight_override(profile_name: str, weights_map: dict) -> float:
    ...
    try:
        exact = float((weights_map or {}).get(profile_name, 0) or 0)
        if exact > 0:
            return exact
    except Exception:
        pass
    ...
    for k, v in (weights_map or {}).items():
        ...
        if val <= 0:
            continue
        if profile_upper == key or profile_upper.startswith(key):
            if len(key) > best_len:
                best = val
                best_len = len(key)
...
def lookup_profile_cost_per_kg(profile_name: str) -> float:
    ...
    return float(st.session_state.get("aluminium_cost_per_kg_value", 0.0) or 0)


def build_profile_cost_map_rows(profile_rows, weights_map):
    names = sorted({str(r.get("profile", "")).strip() for r in profile_rows if str(r.get("profile", "")).strip()})
    existing = get_profile_cost_map()
    rows = []
    for profile in names:
        existing_row = existing.get(profile, {})
        default_weight = existing_row.get("weight_per_m_kg", 0) or lookup_profile_weight_override(profile, weights_map)
        default_cost = existing_row.get("cost_per_kg", 0) or float(st.session_state.get("aluminium_cost_per_kg_value", 0.0) or 0)
        rows.append({
            "profile": profile,
...
def build_project_material_dashboard(summary: dict, stock_length_mm: float, glass_sheet_width_mm: float, glass_sheet_height_mm: float) -> dict:
    aluminium_rate = float(st.session_state.get("aluminium_cost_per_kg_value", 0.0) or 0.0)
    glass_rate_sqft = float(st.session_state.get("glass_cost_per_sqft_value", 0.0) or 0.0)
    profiles_weight_kg = float(summary.get("estimated_weight_kg", 0) or 0.0)
    profiles_cost = profiles_weight_kg * aluminium_rate
    sheets_to_purchase = int(summary.get("glass_new_sheets", 0) or 0)
    sheet_area_m2 = (float(glass_sheet_width_mm or 0.0) * float(glass_sheet_height_mm or 0.0)) / 1000000.0
    total_sheet_purchase_area_m2 = sheets_to_purchase * sheet_area_m2
    glass_cost = total_sheet_purchase_area_m2 * 10.7639 * glass_rate_sqft
    return {
        "profiles_weight_kg": round(profiles_weight_kg, 2),
        "profiles_cost": round(profiles_cost, 2),
        "sheets_to_purchase": sheets_to_purchase,
        "glass_purchase_area_m2": round(total_sheet_purchase_area_m2, 2),
        "glass_cost": round(glass_cost, 2),
        "total_material_cost": round(profiles_cost + glass_cost, 2),
    }



def build_schedule_bom_workbook(
    window_schedule_df: pd.DataFrame,
    bom_profiles_df: pd.DataFrame,
    bom_glass_detail_df: pd.DataFrame,
    bom_glass_purchase_df: pd.DataFrame,
) -> bytes:
    output = BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        bom_glass_detail_df.to_excel(writer, sheet_name="BOM Glass Pieces", index=False)
        bom_glass_purchase_df.to_excel(writer, sheet_name="BOM Glass Purchase", index=False)
...
def render_aluminium_bar_layouts(aluminium: dict):
    ...
    for bar in bars:
            ...
            <div class="soft-card">
                <div style="display:flex;justify-content:space-between;gap:12px;align-items:center;flex-wrap:wrap;margin-bottom:10px;">
                    ...
                    <div style="color:#475569;font-size:0.9rem;">
                        ...
                        &nbsp;|&nbsp; Source length: <b>{int(stock)} mm</b>
        ...
        for idx, cut in enumerate(cuts, start=1):
            ...
            segs.append(
                f'<div style="width:{width_pct:.4f}%;min-width:92px;height:108px;background:#dbeafe;'
                'border-right:1px solid white;display:flex;align-items:center;justify-content:center;'
                'text-align:center;font-size:10px;font-weight:700;color:#1e3a8a;padding:4px;overflow:hidden;line-height:1.1;">'
                f'{win}<br>{profile_name[:22]}<br>{int(length)} mm<br><span style="font-size:11px;color:#0f172a;">Cut {angle}</span></div>'
            )
            ...
            if idx < len(cuts):
                ...
                markers.append(
                    f'<div style="position:absolute;left:calc({left_pct:.4f}% - 1px);top:32px;bottom:0;width:2px;background:#0f172a;"></div>'
                    f'<div style="position:absolute;left:calc({left_pct:.4f}% - 28px);top:0;background:#0f172a;color:white;'
                    f'border-radius:999px;padding:4px 9px;font-size:11px;font-weight:800;">{angle}</div>'
                )

        if waste > 0:
            waste_pct = max((waste / stock) * 100.0, 2.2)
            segs.append(
                f'<div style="width:{waste_pct:.4f}%;min-width:58px;height:108px;background:#fee2e2;'
        ...
        st.markdown(
            f"""
            <div style="position:relative;padding-top:32px;margin:-8px 0 12px 0;">
...
with st.sidebar:
    ...
    with p1:
        if st.button("New project", use_container_width=True):
            ...
            st.session_state.glass_sheet_height_mm_value = 2440.0
            st.session_state.kerf_mm_value = 3.0
            st.session_state.aluminium_cost_per_kg_value = 0.0
            st.session_state.glass_cost_per_sqft_value = 0.0
    ...
    aluminium_cost_per_kg = st.number_input("Aluminium cost per kg", min_value=0.0, step=0.1, key="aluminium_cost_per_kg_value")
    glass_cost_per_sqft = st.number_input("Glass cost per sq ft", min_value=0.0, step=0.1, key="glass_cost_per_sqft_value")
    if "default_row_glass_value" not in st.session_state or st.session_state.default_row_glass_value not in glass_specs:
        st.session_state.default_row_glass_value = glass_specs[0] if glass_specs else ""
    default_row_glass = st.selectbox("Default glass specification", glass_specs, key="default_row_glass_value")
...
tab0, tab1, tab2, tab3, tab4, tab5 = st.tabs(["🏗 Project Board", "🪟 Window Studio", "♻ Offcuts", "📊 Results", "✂ Cutting Layouts", "📦 Schedule & BOM"])
...
# Ensure costing and summary datasets exist before Results and Schedule/BOM tabs use them
if "profile_cost_map" not in st.session_state:
    st.session_state.profile_cost_map = []

if not profile_totals.empty:
    profile_totals["weight_per_m_kg"] = profile_totals["profile"].apply(lambda p: lookup_profile_weight_override(p, getattr(catalog, "weights", {})))
    profile_totals["cost_per_kg"] = profile_totals["profile"].apply(lambda p: lookup_profile_cost_per_kg(p))
    profile_totals["estimated_weight_kg"] = profile_totals.apply(
        lambda r: round(float(r.get("total_cut_length_m", 0) or 0) * float(r.get("weight_per_m_kg", 0) or 0), 2),
        axis=1,
    )
    profile_totals["estimated_cost"] = profile_totals.apply(
        lambda r: round(float(r.get("estimated_weight_kg", 0) or 0) * float(r.get("cost_per_kg", 0) or 0), 2),
        axis=1,
    )
    missing_weight_profiles = profile_totals.loc[profile_totals["weight_per_m_kg"].fillna(0) <= 0, "profile"].astype(str).tolist()
else:
    profile_totals["weight_per_m_kg"] = []
    profile_totals["cost_per_kg"] = []
    profile_totals["estimated_weight_kg"] = []
    profile_totals["estimated_cost"] = []
    missing_weight_profiles = []

glass_piece_df = pd.DataFrame(glass_rows)
if not glass_piece_df.empty and "spec" in glass_piece_df.columns:
    if "qty" not in glass_piece_df.columns:
        glass_piece_df["qty"] = 1
    if "area_m2" not in glass_piece_df.columns:
        if "width_mm" in glass_piece_df.columns and "height_mm" in glass_piece_df.columns:
            glass_piece_df["area_m2"] = (
                glass_piece_df["width_mm"].fillna(0).astype(float)
                * glass_piece_df["height_mm"].fillna(0).astype(float)
                / 1000000.0
            )
        else:
            glass_piece_df["area_m2"] = 0.0
    glass_summary = (
        glass_piece_df.groupby("spec", dropna=False)
        .agg(
            total_pieces=("qty", "sum"),
            total_area_m2=("area_m2", lambda s: float((s * glass_piece_df.loc[s.index, "qty"]).sum())),
        )
        .reset_index()
    )
else:
    glass_summary = pd.DataFrame(columns=["spec", "total_pieces", "total_area_m2"])

aluminium_cost_total = float(profile_totals["estimated_cost"].sum()) if "estimated_cost" in profile_totals.columns and not profile_totals.empty else 0.0
if "glass_cost_per_sqft_value" in st.session_state:
    glass_cost_total = float(summary.get("total_glass_area_m2", 0) or 0) * 10.7639 * float(st.session_state.get("glass_cost_per_sqft_value", 0.0) or 0)
else:
    glass_cost_total = float(summary.get("total_glass_area_m2", 0) or 0) * float(st.session_state.get("glass_cost_per_m2_value", 0.0) or 0)

if not glass_summary.empty:
    if "glass_cost_per_sqft_value" in st.session_state:
        glass_summary["estimated_cost"] = glass_summary["total_area_m2"].apply(lambda x: round(float(x) * 10.7639 * float(st.session_state.get("glass_cost_per_sqft_value", 0.0) or 0), 2))
    else:
        glass_summary["estimated_cost"] = glass_summary["total_area_m2"].apply(lambda x: round(float(x) * float(st.session_state.get("glass_cost_per_m2_value", 0.0) or 0), 2))
else:
    glass_summary["estimated_cost"] = []
...
with tab3:
    ...
    if not profile_rows and not glass_rows:
        st.error("No calculated profile or glass rows were produced from the current window inputs. Check the required inputs for the selected variant.")

    project_material_dashboard = build_project_material_dashboard(
        summary,
        stock_length_mm,
        glass_sheet_width_mm,
        glass_sheet_height_mm,
    ...
    st.markdown('<div class="section-title">Project material dashboard</div>', unsafe_allow_html=True)
    st.caption("High-level material totals for quick project review. This dashboard is independent of the detailed Results and BOM sections below.")
    d1, d2, d3 = st.columns(3)
    with d1:
        render_metric("Profiles weight", f"{project_material_dashboard['profiles_weight_kg']} kg")
        st.markdown(
            f"<div style='margin-top:8px;color:#475569;font-size:0.92rem;'><b>Profiles cost:</b> {money_fmt(project_material_dashboard['profiles_cost'])}</div>",
            unsafe_allow_html=True,
        )
    with d2:
        render_metric("Glass sheets", str(project_material_dashboard["sheets_to_purchase"]))
        st.markdown(
            f"<div style='margin-top:8px;color:#475569;font-size:0.92rem;'><b>Glass purchase area:</b> {project_material_dashboard['glass_purchase_area_m2']} m²</div>",
            unsafe_allow_html=True,
        )
        st.markdown(
            f"<div style='margin-top:4px;color:#475569;font-size:0.92rem;'><b>Glass cost:</b> {money_fmt(project_material_dashboard['glass_cost'])}</div>",
            unsafe_allow_html=True,
        )
    with d3:
        render_metric("Total material", money_fmt(project_material_dashboard["total_material_cost"]))
        st.markdown(
            f"<div style='margin-top:8px;color:#475569;font-size:0.92rem;'><b>Aluminium rate:</b> {money_fmt(st.session_state.get('aluminium_cost_per_kg_value', 0.0))} / kg</div>",
            unsafe_allow_html=True,
        )
        st.markdown(
            f"<div style='margin-top:4px;color:#475569;font-size:0.92rem;'><b>Glass rate:</b> {money_fmt(st.session_state.get('glass_cost_per_sqft_value', 0.0))} / sq ft</div>",
            unsafe_allow_html=True,
        )
    st.markdown('</div>', unsafe_allow_html=True)

    st.markdown('<div class="soft-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">Project calculation summary</div>', unsafe_allow_html=True)
    st.caption("Review the whole project first, then profiles, then glass, then purchasing totals.")

    m1, m2, m3, m4 = st.columns(4)
    with m1:
        render_metric("Window lines", str(summary.get("window_lines", 0)))
    with m2:
        render_metric("Profile cuts", str(summary.get("profile_cuts", 0)))
    with m3:
        render_metric("New aluminium bars", str(summary.get("aluminium_new_bars", 0)))
    with m4:
        render_metric("New glass sheets", str(summary.get("glass_new_sheets", 0)))

    with st.expander("Calculation preview", expanded=False):
        pprev = pd.DataFrame(profile_rows)
        gprev = pd.DataFrame(glass_rows)
        c1, c2 = st.columns(2)
        with c1:
            st.markdown("**Calculated profile rows**")
            if not pprev.empty:
                cols = [c for c in ["window_label", "profile", "length_mm", "qty", "cut_degree"] if c in pprev.columns]
                st.dataframe(pprev[cols], use_container_width=True, hide_index=True)
            else:
                st.info("No profile rows were produced.")
        with c2:
            st.markdown("**Calculated glass rows**")
            if not gprev.empty:
                cols = [c for c in ["window_label", "spec", "width_mm", "height_mm", "qty"] if c in gprev.columns]
                st.dataframe(gprev[cols], use_container_width=True, hide_index=True)
            else:
                st.info("No glass rows were produced.")
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="soft-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">Profiles — material and purchasing</div>', unsafe_allow_html=True)
    p1, p2, p3 = st.columns(3)
    with p1:
        st.metric("Total profile length", f"{mm_to_m(summary.get('total_profile_length_mm', 0))} m")
    with p2:
        st.metric("Estimated aluminium weight", f"{round(float(summary.get('estimated_weight_kg', 0) or 0), 2)} kg")
    with p3:
        st.metric("Estimated aluminium purchase cost", money_fmt(aluminium_cost_total))
    ...
    if not profile_totals.empty:
        ...
        st.dataframe(profile_totals[show_cols], use_container_width=True, hide_index=True)
    else:
        st.info("No weighted profile summary available.")

    st.markdown("**Profile lengths to order after offcuts**")
    ...
    st.markdown('<div class="soft-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">Glass — material and purchasing</div>', unsafe_allow_html=True)
    g1, g2, g3 = st.columns(3)
    with g1:
        st.metric("Total glass area", f"{round(float(summary.get('total_glass_area_m2', 0) or 0), 2)} m²")
    with g2:
        st.metric("Glass offcut hits", str(summary.get("glass_offcut_hits", 0)))
    with g3:
        st.metric("Estimated glass purchase cost", money_fmt(glass_cost_total))
    ...
    st.markdown("**Glass optimiser placements from new sheets**")
    ...
    with m1:
        st.metric("Aluminium purchase cost", money_fmt(aluminium_cost_total))
    with m2:
        st.metric("Glass purchase cost", money_fmt(glass_cost_total))
    with m3:
        st.metric("Total material cost", money_fmt(float(aluminium_cost_total or 0) + float(glass_cost_total or 0)))
...
with tab5:
    ...
    bom_profiles_df = order_breakdown.copy() if 'order_breakdown' in locals() and isinstance(order_breakdown, pd.DataFrame) else pd.DataFrame(columns=["profile", "bars_to_order", "ordered_length_mm", "ordered_length_m", "used_length_mm", "used_length_m", "waste_mm", "waste_m"])
    if not bom_profiles_df.empty:
        if "bars_to_order" in bom_profiles_df.columns and "lengths_to_order" not in bom_profiles_df.columns:
            bom_profiles_df["lengths_to_order"] = bom_profiles_df["bars_to_order"]
        bom_profiles_df["weight_per_m_kg"] = bom_profiles_df["profile"].apply(lambda p: lookup_profile_weight_override(p, getattr(catalog, "weights", {})))
        bom_profiles_df["cost_per_kg"] = bom_profiles_df["profile"].apply(lambda p: lookup_profile_cost_per_kg(p))
        bom_profiles_df["estimated_weight_kg"] = bom_profiles_df.apply(lambda r: round(float(r.get("ordered_length_m", 0) or 0) * float(r.get("weight_per_m_kg", 0) or 0), 2), axis=1)
        bom_profiles_df["estimated_cost"] = bom_profiles_df.apply(lambda r: round(float(r.get("estimated_weight_kg", 0) or 0) * float(r.get("cost_per_kg", 0) or 0), 2), axis=1)
    ...
    if not glass_sheet_piece_df.empty:
        ...
        glass_sheet_piece_df["cut_width_mm"] = glass_sheet_piece_df["placed_width_mm"] if "placed_width_mm" in glass_sheet_piece_df.columns else glass_sheet_piece_df["width_mm"]
        glass_sheet_piece_df["cut_height_mm"] = glass_sheet_piece_df["placed_height_mm"] if "placed_height_mm" in glass_sheet_piece_df.columns else glass_sheet_piece_df["height_mm"]
        glass_sheet_piece_df["piece_area_m2"] = glass_sheet_piece_df["cut_width_mm"].astype(float).fillna(0) * glass_sheet_piece_df["cut_height_mm"].astype(float).fillna(0) / 1000000.0
        glass_sheet_piece_df["qty"] = 1
        bom_glass_detail_df = glass_sheet_piece_df.copy()
    else:
        bom_glass_detail_df = pd.DataFrame(columns=["piece_id", "window_label", "spec", "sheet_no", "cut_width_mm", "cut_height_mm", "piece_area_m2"])

    # Calculate glass purchasing from total sheet area purchased, not from total piece area
    purchased_sheet_count = int(len(set(bom_glass_detail_df["sheet_no"].dropna().astype(str).tolist()))) if not bom_glass_detail_df.empty and "sheet_no" in bom_glass_detail_df.columns else int(summary.get("glass_new_sheets", 0) or 0)
    sheet_area_m2 = float(glass_sheet_width_mm or 0) * float(glass_sheet_height_mm or 0) / 1000000.0
    total_sheet_purchase_area_m2 = purchased_sheet_count * sheet_area_m2
    glass_rate_sqft = float(st.session_state.get("glass_cost_per_sqft_value", 0.0) or 0)
    total_glass_purchase_cost = total_sheet_purchase_area_m2 * 10.7639 * glass_rate_sqft

    bom_glass_purchase_df = pd.DataFrame([{
        "sheets_to_purchase": purchased_sheet_count,
        "sheet_width_mm": float(glass_sheet_width_mm or 0),
        "sheet_height_mm": float(glass_sheet_height_mm or 0),
        "sheet_area_m2": round(sheet_area_m2, 4),
        "total_sheet_purchase_area_m2": round(total_sheet_purchase_area_m2, 4),
        "glass_cost_per_sqft": round(glass_rate_sqft, 2),
        "estimated_cost": round(total_glass_purchase_cost, 2),
    }])
    ...
    if not bom_profiles_df.empty:
        ...
        st.info("No profile purchase BOM available.")
    st.markdown('</div>', unsafe_allow_html=True)

    st.markdown('<div class="soft-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">Bill of Materials — Glass pieces to cut from new sheets</div>', unsafe_allow_html=True)
    st.caption("Shows each glass piece that still needs to be cut from purchased sheets after glass offcuts have been deducted.")
    if not bom_glass_detail_df.empty:
        cols = [c for c in ["piece_id", "window_label", "spec", "sheet_no", "cut_width_mm", "cut_height_mm", "piece_area_m2"] if c in bom_glass_detail_df.columns]
        st.dataframe(bom_glass_detail_df[cols], use_container_width=True, hide_index=True)
    else:
        st.info("No glass needs to be cut from new sheets. All glass pieces were covered by offcuts.")
    st.markdown('</div>', unsafe_allow_html=True)

    st.markdown('<div class="soft-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">Glass purchasing based on total sheet area</div>', unsafe_allow_html=True)
    st.caption("Glass cost is calculated from the total area of full sheets that must be purchased, not from the combined area of the cut pieces.")
    st.dataframe(bom_glass_purchase_df, use_container_width=True, hide_index=True)
    st.markdown('</div>', unsafe_allow_html=True)

    total_profiles_cost = float(bom_profiles_df["estimated_cost"].sum()) if "estimated_cost" in bom_profiles_df.columns and not bom_profiles_df.empty else 0.0
    total_glass_cost = float(bom_glass_purchase_df["estimated_cost"].sum()) if not bom_glass_purchase_df.empty else 0.0

    st.markdown('<div class="soft-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">Schedule & BOM Export</div>', unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    with c1:
        st.metric("Profiles BOM cost", money_fmt(total_profiles_cost))
    with c2:
        st.metric("Glass BOM cost", money_fmt(total_glass_cost))
    with c3:
        st.metric("Combined BOM cost", money_fmt(total_profiles_cost + total_glass_cost))

    workbook_bytes = build_schedule_bom_workbook(window_schedule_df, bom_profiles_df, bom_glass_detail_df, bom_glass_purchase_df)
    st.download_button(
