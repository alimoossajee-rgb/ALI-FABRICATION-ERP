...
def safe_eval(expr: str) -> float:
    def _eval(node):
        ...
        if isinstance(node, ast.Constant):
            return node.value
        # ast.Num was removed in newer Python versions used by Streamlit Cloud.
        # Keep compatibility without directly referencing ast.Num.
        if getattr(ast, "Num", None) is not None and isinstance(node, getattr(ast, "Num")):  # pragma: no cover
            return node.n
