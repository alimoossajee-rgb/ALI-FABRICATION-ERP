...
def optimise_glass(
    ...
    def _offcut_candidate_score(piece: Dict[str, Any], offcut: Dict[str, Any], rotated: bool) -> tuple:
        ...
        # Simple best-fit:
        # 1) smallest leftover area after the cut
        # 2) smallest source offcut area
        # 3) smallest leftover longest side
        leftover_area = max(0.0, source_w * source_h - pw * ph)
        source_area = source_w * source_h
        longest_leftover_side = max(max(0.0, source_w - pw), max(0.0, source_h - ph))

        return (
            round(leftover_area, 4),
            round(source_area, 4),
            round(longest_leftover_side, 4),
        )
