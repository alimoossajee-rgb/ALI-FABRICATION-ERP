...
def optimise_glass(
    ...
    def _offcut_candidate_score(piece: Dict[str, Any], offcut: Dict[str, Any], rotated: bool) -> tuple:
        pw = piece["height_mm"] if rotated else piece["width_mm"]
        ph = piece["width_mm"] if rotated else piece["height_mm"]
        source_w = offcut["width_mm"]
        source_h = offcut["height_mm"]

        right_w = max(0.0, source_w - pw - kerf_mm)
        top_h = max(0.0, source_h - ph - kerf_mm)

        # Resulting guillotine remainders
        right_area = right_w * ph if right_w > 0 and ph > 0 else 0.0
        top_area = source_w * top_h if top_h > 0 and source_w > 0 else 0.0
        total_leftover_area = right_area + top_area

        # Penalize unusable slivers so we don't burn a good offcut for a bad remainder.
        sliver_penalty = 0
        if right_w > 0 and right_w < min_useful_mm:
            sliver_penalty += 1
        if top_h > 0 and top_h < min_useful_mm:
            sliver_penalty += 1

        # Prefer exact/smaller fits and better reusable remainder shapes.
        max_remainder_area = max(right_area, top_area)
        source_area = source_w * source_h
        short_side_penalty = min(right_w if right_w > 0 else 10**9, top_h if top_h > 0 else 10**9)

        return (
            sliver_penalty,
            round(total_leftover_area, 4),
            round(max_remainder_area, 4),
            round(source_area, 4),
            round(short_side_penalty, 4),
        )
    ...
    for piece in pieces:
        ...
        for offcut in leftover_offcuts:
            ...
            if fits:
                score = _offcut_candidate_score(piece, offcut, rotated)
                candidates.append((score, offcut, rotated))
        if candidates:
            ...
            # simple guillotine split using best-fit chosen offcut
